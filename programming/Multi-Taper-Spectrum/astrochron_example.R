### EXAMPLES OF SOME FUNCTIONS AVAILABLE IN THIS SOFTWARE:### This demo will use a model (series are usually read using the function'read').
data(modelA)
...
### In this extracted spectrum, identify F-test peak maxima exceeding 90% confidence 
levelfreqs=peak(spec,level=0.9)[2]

### Conduct ASM testing on these peaks
# set Rayleigh frequency in cycles/m
#Rayleigh frequency (the lowest positive frequency in the spectrum)
rayleigh=0.1245274
# set Nyquist frequency in cycles/m
#Nyquist frequency (the largest frequency in the spectrum)
nyquist=6.66597
# set orbital target in 1/ky
target=c(1/405.47,1/126.98,1/96.91,1/37.66,1/22.42,1/18.33)
# execute ASM
asm(freq=freqs,target=target,rayleigh=rayleigh,nyquist=nyquist,
    sedmin=0.5,sedmax=3,numsed=100,linLog=1,iter=100000,output=FALSE)
...
if(interactive()) {
  ...
  ### Convert the sedimentation rate curve to a time-space 
  maptime=sedrate2time(sedrate)
