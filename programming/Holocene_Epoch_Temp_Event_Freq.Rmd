...
process_data <- function(data_file_name, col_num) 
{  
  # read the data file  
  ...
  data_cols <- strsplit(data_cols, " ")[[1]] 
...
  # generate the data frame after string processing, basically collecting the column data  
  ...
  {    
   ...
   # set the column name  
   colnames(data_frame) <- stei_data_cols    
   return(data_frame)
 }
```
...
cur_dir <- getwd()
t_dir <- "/Users/andy/Dropbox/TSCreator/TSCreator development/Developers/Andy/Projects/ML-Data Mining/databases/external_data/"setwd(t_dir)
...
legend("topright", col=c("black", "orange", "blue", "red"), lty=c(1, 1, 1, 1), c("Normalized event frequency", "Rolling avg of event frequency", "Temperatue(°C)", "Rolling avg of temperature(°C)"), bg="white", cex=.6)
```
...
CC <- ts(ev, start=0, end=1, frequency=120)  # 12000/100 = 120
par(mfrow=c(1,2))
