...
families <- c(
  ...
  "Globigerinidae", # old(BFG): "Catapsydrax" http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=721443      
  "Globigerinidae", # old(BFG): "Globorotaliidae", # "Clavatorella" http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=721438       
  ...
  "Globigerinidae", # old(BFG): Eoglobigerinidae "Eoglobigerina" http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=721429
  ...
  "Globoquadrinidae", # old (BFG): "Catapsydracidae", # Globoquadrina
  "Globorotaliidae", # Globorotalia
  "Globigerinidae", # Globorotaloides
  "Globigerinidae", # Globoturborotalita
  "Globigerinidae", # old (BFG): "Catapsydracidae", # "Guembelitrioides" http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=1035049
  ...
  "Globorotaliidae", # old (BFG): "Bittnerulidae", # Hirsutella
  ...
  "Globigerinidae", # old (BFG): "Catapsydracidae", # Protentella http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=827109
  "Globigerinidae", # old (BFG): "Catapsydracidae", # "Protentelloides"     http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=1063500
  ...
  "Globigerinidae", # old (BFG): "Catapsydracidae", # "Subbotina" http://www.marinespecies.org/foraminifera/aphia.php?p=taxdetails&id=932735
...
datatable(dff)
write.table(dff, "foram_turnover_cenozoic.csv", sep=",")
...
PF_FAD_LAD_per_df_ap <- FAD_LAD_per_df
head(PF_FAD_LAD_per_df_ap)

par(mfrow=c(2,1))
par(mar=c(1,4,1,4))
width <- 10 # to obtain every million year
by <- 3
x <- rollapply(-PF_FAD_LAD_per_df_ap$age, width=width, by=by, FUN='mean')
y <- rollapply(PF_FAD_LAD_per_df_ap$LAD_cnt + PF_FAD_LAD_per_df_ap$FAD_cnt,
               width=width, by=by, FUN='mean')
x <- -PF_FAD_LAD_per_df_ap$age
y <- PF_FAD_LAD_per_df_ap$LAD_cnt + PF_FAD_LAD_per_df_ap$FAD_cnt
plot(x, 
     y, 
     t='l', 
     lwd=2,
...
y1 <- rollapply(PF_FAD_LAD_per_df_ap$LAD_cnt,
                width=width, by=by, FUN='mean')
y1 <- PF_FAD_LAD_per_df_ap$LAD_cnt
lines(x, y1, #ages, extinction_rate, 
      lwd=2,
      col=2)
#points(x, y1, cex=0.5, col='red')

y2 <- rollapply(PF_FAD_LAD_per_df_ap$FAD_cnt,
                width=width, by=by, FUN='mean')
y2 <- PF_FAD_LAD_per_df_ap$FAD_cnt
...
yd <- rollapply(pf_diversity_per_level$N.species.speciation,
               width=width, by=by, FUN='mean')
xd <- -PF_dff$age
yd <- PF_dff$N.species.speciation
par(new=T)
