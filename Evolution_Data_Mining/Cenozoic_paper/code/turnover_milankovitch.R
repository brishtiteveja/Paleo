# obliquity
df = eccentricity_df
df2 = data.frame(x=NN_dff$age, y=NN_dff$raw.turnover.probability)
df2 = na.omit(df2)
df2 = df2[df2$x<=max(df$age),]
plot(df2, t='l')
mtm(df2, ar1=F, output = 4, pl=3, siglevel=0.95)

par(mfrow=c(1,1))
plot(df, t='l')
# filtered 1.18 myr component
f <- 1/1.18
flow <- f - 0.1
fhigh <- f + 0.1

#m1 <- mtm(df, output = 4)
m2 <- mtm(df, ar1=F, output = 4, pl=3, siglevel=0.99)
m3 <- mtm(df2, ar1=F, output = 4, pl=3, siglevel=0.95)
#m2 <- mtm(eccentricity_df, ar1=F, output=4, pl=3)

#1   0.2352775   4.2503   94.89745 
#2   0.8411171   1.188895   99.96137 
#3   1.523422   0.656417   90.75292 
#4   2.435122   0.410657   92.03912 

xmin <- 1/4
xmax <- 1/1.5

id <- m2$freq > xmin & m2$freq < xmax
...
abline(v=1/2.40, lty=2, lwd=2, col='green')
abline(v=1/1.20, lty=2, lwd=2, col='green')
abline(v=1/0.97, lty=2, lwd=2, col='green')


plot(m2$freq, log(m2$spec), t='l')

id <- m$freq > flow & m$freq < fhigh
id
freq <- m$freq[id]
power <- 1/freq
hm <- m$mtm$Ftest[id]
...
ob_1.2myr <- bandpass(obliquity_df,
                      flow = flow,
                      fhigh = fhigh,
                      win = 1,
                      output = 1)
...
par(mfrow=c(1,1))
plot(obliquity_df, t='l', xlim=c(2,24))
par(new=T)
plot(ob_2.4myr, col='green', t='l', xlim=c(2,24))
